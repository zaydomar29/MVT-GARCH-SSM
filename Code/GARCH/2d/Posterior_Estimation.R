...
##################################################################
##################################################################
Rcpp::sourceCpp("./Functions/MvtKalFiltGarch.cpp")
Rcpp::sourceCpp("./Functions/MvtKalFFBS.cpp")
Rcpp::sourceCpp("./Functions/MvtLogPost.cpp")
...
out = MVTKalFiltGarchC(Y=Y,GG=GG,FF=FF,W=W.old,m0=Y[1,],C0=C0,garch=old_param,cor=rho.old)
theta = do.call(rbind,lapply(MVTKalffbsC(Y, out)$theta,t))



old_lgpost = MVT_lgpostSS(par = old_param,cor = rho.old,W = W.old,Y = Y, theta = theta, nt = 10, psi = diag(10,2))
...
for(it in 1:sims){
  ...
  if(it %% 100 == 0){print(rho.old)}
  if(it %% 100 == 0){print(W.old)}
  if(it %% 100 == 0){print(paste("acc.rate.a0:",acc.rate.a0/it))}
  if(it %% 100 == 0){print(paste("acc.rate.a1:",acc.rate.a1/it))}
  if(it %% 100 == 0){print(paste("acc.rate.b1:",acc.rate.b1/it))}
  if(it %% 1000 == 0){
    par(mfrow = c(1,1))
  ...
  for(j in 1:2){
    ...
    rho = MVT_lgpostSS(par = new_param,cor = rho.old,W = W.old,Y = Y, theta = theta, nt = 10, psi = diag(10,2))+
      log(dtruncnorm(old_param[j,1],a=0,b=Inf,mean=a0.prop,0.1))-
      MVT_lgpostSS(par = old_param,cor = rho.old,W = W.old,Y = Y, theta = theta, nt = 10, psi = diag(10,2))-
  ...
  rho = MVT_lgpostSS(par = old_param,cor = rho.prop,W = W.old,Y = Y, theta = theta, nt = 10, psi = diag(1,2))-
        MVT_lgpostSS(par = old_param,cor = rho.old,W = W.old,Y = Y, theta = theta, nt = 10, psi = diag(1,2))
