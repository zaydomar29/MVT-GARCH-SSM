...
model_par = rbind(c(1,0.7,0.1,0.05),
                  c(0.5,0.1,0.6,0.05),
                  c(3,0,0,0.05),
                  c(4,0,0,0.05))
...
## Inspect the GARCH-(1,1) error terms
par(mfrow = c(1,1))
...
## Generate the state vectors and observed data
state = matrix(c(10.6,-50.5,0,-10), nc = p, nr = n+1,byrow = T)
state_error = matrix(c(10.6,-50.5,0,-10), nc = p, nr = n,byrow = T)
Y = matrix(0,nr=n,nc=p)
for(i in 1:n){
  state_error[i,] = as.vector(chol(diag(model_par[1:p,4]))%*%rnorm(p))
